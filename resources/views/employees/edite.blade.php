...
<div class="container mx-auto p-4 max-w-3xl">
    <h2 class="text-2xl font-bold mb-6">Edit Employee - {{ $employee->name }}</h2>

    <form action="{{ route('employees.update', $employee) }}" method="POST" class="space-y-6 bg-white p-6 rounded shadow">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="name" id="name" 
                class="w-full border border-gray-300 rounded px-3 py-2" value="{{ old('name', $employee->name) }}" required>
        ...
        <div>
            ...
            <input type="email" name="email" id="email" 
                class="w-full border border-gray-300 rounded px-3 py-2" value="{{ old('email', $employee->email) }}" required>
        ...
        <div>
            ...
            <input type="text" name="position" id="position" 
                class="w-full border border-gray-300 rounded px-3 py-2" value="{{ old('position', $employee->position) }}" required>
        ...
        <div>
            ...
            <select name="country" id="country" class="w-full border border-gray-300 rounded px-3 py-2" required>
                ...
                <option value="UK" {{ old('country', $employee->country) == 'UK' ? 'selected' : '' }}>UK</option>
        ...
        <div>
            ...
            <select name="state" id="state" class="w-full border border-gray-300 rounded px-3 py-2" required>
                ...
                <option value="California" {{ old('state', $employee->state) == 'California' ? 'selected' : '' }}>California</option>
                <option value="Maharashtra" {{ old('state', $employee->state) == 'Maharashtra' ? 'selected' : '' }}>Maharashtra</option>
                <option value="London" {{ old('state', $employee->state) == 'London' ? 'selected' : '' }}>London</option>
        ...
        <div>
            ...
            <select name="city" id="city" class="w-full border border-gray-300 rounded px-3 py-2" required>
                ...
                <option value="Los Angeles" {{ old('city', $employee->city) == 'Los Angeles' ? 'selected' : '' }}>Los Angeles</option>
                <option value="Mumbai" {{ old('city', $employee->city) == 'Mumbai' ? 'selected' : '' }}>Mumbai</option>
                <option value="London City" {{ old('city', $employee->city) == 'London City' ? 'selected' : '' }}>London City</option>
